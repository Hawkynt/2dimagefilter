...
namespace Classes {
  /// <summary>
  /// Runtime POCO that surfaces a manipulator's <see cref="ParameterDescriptor"/> set as a
  ...
  /// subclass is sufficient and works on every TFM the consumer targets (net48 today,
  /// modern .NET tomorrow).
  /// </para>
  /// <para>
  ...
  internal sealed class ManipulatorParameterBag : ICustomTypeDescriptor {
    ...
    private ManipulatorParameterBag(IReadOnlyList<ParameterDescriptor> descriptors) {
    ...
    public static ManipulatorParameterBag CreateFor(IReadOnlyList<ParameterDescriptor> descriptors) {
      ...
      return new ManipulatorParameterBag(descriptors);
    }
