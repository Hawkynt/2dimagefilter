/**
 * @file basic.h
 * @brief Memory management, portable types, timing, and math constants
 * @author Pascal Getreuer <getreuer@gmail.com>
 *
 * The purpose of this file is to gather useful nonportable C features
 * and then use compiler-defined symbols so that the right thing happens
 * most of the time automatically.  This way, if changes are required
 * for your platform, hopefully only this file and possibly also basic.c
 * need to be changed.
 *
 * For memory management, macros Malloc, Realloc, and Free are defined
 * here, which can be modified to customize memory management.
 *
 * Types \c uint8_t, \c uint16_t, \c uint32_t are defined as unsigned
 * integer types such that
 ...
 *
 * Similarly, \c int8_t, \c int16_t, \c int32_t are defined as signed
 * integer types such that
 ...
 *
 * A millisecond-precision timer Clock() is defined.  There is no
 * portable ANSI C millisecond timer, so Clock is defined depending on
 * the platform.  On Windows systems, Clock is implemented using
 * GetSystemTime.  On POSIX systems, Clock uses gettimeofday.  If the
 * system is neither POSIX nor Windows, the time.h time function is
 * used as a fallback, which has only second accuracy.
 *
...
/* Use stdint to define uint8_t, etc. */
#include <stdint.h>
...
/**
 * @brief Time in milliseconds, useful for measuring elapsed time
 * @return Number of milliseconds since an arbitrary point
 *
 * This routine obtains the system time with millisecond precision.  There
 * is no portable ANSI C millisecond timer, so Clock is defined depending on
 * the platform.  On POSIX systems, Clock uses gettimeofday.  Otherwise,
 * if the system is Windows, GetSystemTime is used.  If the system is neither
 * POSIX nor Windows, the low-resolution time.h clock function is used as a
 * fallback.
 *
