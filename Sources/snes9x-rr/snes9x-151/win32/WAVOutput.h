...
#ifndef __WAVOUTPUT_H_
#define __WAVOUTPUT_H_

#ifdef __cplusplus
extern "C" {
#endif

// create a new wav output object
void WAVCreate(struct WAVFile** wav_out);

// set up wav output options
...
int WAVBegin(const char* filename, struct WAVFile* wav_out);

// get the format in use from an already existing wav output object
// returns 1 if successful, 0 otherwise
int WAVGetSoundFormat(const struct WAVFile* wav_out, const WAVEFORMATEX** ppFormat);

// write sound samples to an wav output object
